...
export function resolveDependencies(
  ...
  const cycles = detectCycles(dependencyGraph, Array.from(addonMap.keys()))
  ...
  const orderedAddons = topologicalSort(dependencyGraph, addonMap)
 ...
 */
export function getDependencyErrorMessage(result: ResolvedOrder): string {
  if (!result.hasCycles) {
    return ''
  }
